...
export default async function AdminOrderDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {delivery ? <Card><CardHeader><CardTitle>Tài khoản đã cấp</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><p>Tài khoản: <strong>{delivery.username}</strong></p><p>Mật khẩu: <strong>{delivery.password}</strong></p><p>Ghi chú: {delivery.note}</p><div className="flex gap-2"><CopyButton value={delivery.username} label="tài khoản" /><CopyButton value={delivery.password} label="mật khẩu" /></div></CardContent></Card> : null}
    </div>
  );
}
