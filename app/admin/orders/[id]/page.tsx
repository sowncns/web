...
export default async function AdminOrderDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>Thông tin</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><p>Sản phẩm: {order.products?.name}</p><p>Số lượng: <strong>{order.quantity || 1}</strong></p><p>Khách: {order.customer_name} - {order.customer_email} - {order.customer_phone}</p><p>Tổng: {formatCurrency(order.total_amount)}</p><p>Ngày: {formatDate(order.created_at)}</p><div className="flex gap-2"><OrderStatusBadge status={order.payment_status} /><OrderStatusBadge status={order.order_status} /></div></CardContent></Card>
      {order.payment_status === "PAID" && order.order_status !== "COMPLETED" ? <OrderAdminActions orderId={order.id} quantity={Number(order.quantity || 1)} /> : null}
