...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  let query = supabase
    .from("orders")
    .select("id,user_id,order_code,total_amount,payment_status,order_status,quantity,delivery_username_encrypted,delivery_password_encrypted,delivery_note_encrypted,created_at,products(id,name,slug)")
    .eq("id", params.id);
  ...
  const { data: deliveryRows = [] } = canViewDelivery ? await supabase.from("order_deliveries").select("username_encrypted,password_encrypted,note_encrypted").eq("order_id", order.id).order("created_at") : { data: [] };
